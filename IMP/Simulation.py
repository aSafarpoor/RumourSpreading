...
def Simulation_Charlotte3(our_graph, num_red, k, dict_args, dict_counter_measure):
    if dict_counter_measure['id'] == COUNTER_MEASURE_FACTCHECKING:
        graylist, n = Simulation_Charlotte_factchecking(graph=our_graph, num_red=num_red, k=k,
                                                        dict_args=dict_args, dict_counter_measure=dict_counter_measure)
        return graylist, n
    else:
        our_graph = GetInitialOpinions(graph=our_graph, num_red=num_red, dict_args=dict_args,
        ...
        cur_num_red = num_red
        if dict_counter_measure['id'] == COUNTER_MEASURE_ORANGE_NODES:
            initial_num_gray = len(
                [node for node in our_graph.nodes if our_graph.nodes[node]['vote'] == NODE_COLOR_GRAY])
            cur_num_gray = 0
            cur_num_white = n - initial_num_gray - cur_num_red
        if dict_counter_measure['id'] == COUNTER_MEASURE_GREEN_NODES:
            cur_num_green = dict_counter_measure['num_green']
        ...
        if dict_counter_measure["id"] == COUNTER_MEASURE_COMMUNITY_DETECTION:
            threshold_detection = dict_counter_measure["threshold_detection"]
            threshold_block = dict_counter_measure["threshold_block"]
            communities = nx_comm.louvain_communities(our_graph)
            print('num communities', len(communities))
            com_index = 0
            sum_blocked = 0
            for com in communities:
                for node in com:
                    our_graph.nodes[node]["comm"] = com_index
        ...
        while stop != 1:
            ...
            for round in range(k + 1):
                ...
                if dict_counter_measure['id'] == COUNTER_MEASURE_COMMUNITY_DETECTION:
                    # in every round we set this back to 0 and start again from scratch
                    reds_in_com = []
                    red_ratio_per_com = []
                    maxes = []
                    if cur_num_red > (n * threshold_detection):
                        # the number of red nodes in the graph is too high so we are going to look inside the clusters and block
                        com_index = 0
                        for com in communities:
                            reds_in_com.append(0)
                            red_ratio_per_com.append(0)
                            # check inside each community
                            # calculate the number of red nodes in each community
                            for nodec in com:
                                if our_graph.nodes[nodec]['vote'] == NODE_COLOR_RED:
                                    reds_in_com[com_index] = reds_in_com[com_index] + 1
                            red_ratio_per_com[com_index] = reds_in_com[com_index] / len(com)
                            if red_ratio_per_com[com_index] > threshold_block:
                                maxes.append(com_index)
                                for nodec in communities[com_index]:
                                    for edge_n in our_graph.edges(nodec):
                                        if ((edge_n[0] == nodec and our_graph.nodes[edge_n[1]]["comm"] !=
                                             our_graph.nodes[nodec][
                                                 "comm"])
                                                or (edge_n[1] == n and our_graph.nodes[edge_n[0]]["comm"] !=
                                                    our_graph.nodes[n]["comm"])):
                                            our_graph.edges[edge_n]["blocked"] += 1
                ...
                if dict_counter_measure['id'] == COUNTER_MEASURE_GREEN_NODES:
                        ...
                        if our_graph.nodes[node]['stamp'] == k:
                            ...
                            for neigh in [neigh for neigh in neighlist if
                                ...
                                if rand < r:
                                    if dict_counter_measure['id'] == COUNTER_MEASURE_ACCURACYFLAG:
                                        coinflip = random.random()
                                        if coinflip < dict_args['p_coinflip']:
                                            our_graph.nodes[neigh]['hit_counter'] = 1 + our_graph.nodes[neigh][
                                                'hit_counter']
                                            hit_nodes.add(neigh)
                                        else:
                                            our_graph.nodes[neigh]['vote'] = NODE_COLOR_WHITE
                                            ...
                                            cur_num_white = cur_num_white - 1
                                    if dict_counter_measure['id'] == COUNTER_MEASURE_COMMUNITY_DETECTION:
                                        if our_graph.nodes[node]["comm"] in maxes or our_graph.nodes[neigh][
                                            'comm'] in maxes:
                                            if our_graph.nodes[neigh]["comm"] != our_graph.nodes[node]["comm"]:
                                                continue
                                            else:
                    ...
                    for node in hit_nodes:
                        if dict_counter_measure["id"] == COUNTER_MEASURE_NONE or dict_counter_measure[
                            'id'] == COUNTER_MEASURE_ACCURACYFLAG \
                                or dict_counter_measure['id'] == COUNTER_MEASURE_ORANGE_NODES or dict_counter_measure[
                            'id'] == COUNTER_MEASURE_COMMUNITY_DETECTION:
                            our_graph.nodes[node]['vote'] = NODE_COLOR_RED
                            change = change + 1
                            cur_num_red = cur_num_red + 1
                            cur_num_white = cur_num_white - 1
...
def Simulation_Charlotte2(our_graph,num_red, k, dict_args, dict_counter_measure):
    if dict_counter_measure['id'] == COUNTER_MEASURE_FACTCHECKING:
        graylist, n = Simulation_Charlotte_factchecking(graph=our_graph, num_red=num_red, k=k,
                                          dict_args=dict_args, dict_counter_measure=dict_counter_measure)
        return graylist, n
                            ...
                            for neigh in [neigh for neigh in neighlist if
                                ...
                                if rand < r:
                                    if dict_counter_measure['id'] == COUNTER_MEASURE_ACCURACYFLAG:
                                        coinflip = random.random()
                                        if coinflip < dict_args['p_coinflip']:
                                            our_graph.nodes[neigh]['hit_counter'] = 1 + our_graph.nodes[neigh]['hit_counter']
                                            hit_nodes.add(neigh)
                                        else:
                                            our_graph.nodes[neigh]['vote'] = NODE_COLOR_GRAY
                                            cur_num_gray = cur_num_gray + 1
                                    ...
                                    if dict_counter_measure['id'] == COUNTER_MEASURE_COMMUNITY_DETECTION:
                                        ...
                                        hit_nodes.add(neigh)
                    for node in hit_nodes:
                        if dict_counter_measure["id"] == COUNTER_MEASURE_NONE or dict_counter_measure['id'] == COUNTER_MEASURE_ACCURACYFLAG \
                                or dict_counter_measure['id'] == COUNTER_MEASURE_ORANGE_NODES or dict_counter_measure['id'] == COUNTER_MEASURE_COMMUNITY_DETECTION:
                            our_graph.nodes[node]['vote'] = NODE_COLOR_RED
                            change = change + 1
                            cur_num_red = cur_num_red + 1
                            cur_num_white = cur_num_white - 1
